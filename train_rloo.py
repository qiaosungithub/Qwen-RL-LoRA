"""
RLOO (REINFORCE Leave-One-Out) Training Script

Usage:
    python train_rloo.py                           # standalone with defaults
    python train_rl.py --config configs/rloo.yaml  # via unified script
"""
...
def train(config: dict) -> str:
    ...
    training_args = RLOOConfig(
        ...
        run_name=f"rloo-{Path(training_config['output_dir']).name}",
    ...
    # Train
    print(f"\n{'='*60}")
    print(f"Training with RLOO (kl_coef={training_config['kl_coef']})")
...
# RLOO-specific overrides
_RLOO_OVERRIDES = {
    "method": "rloo",
    "training": {
        "output_dir": "outputs/qwen-rloo-gsm8k",
...
DEFAULT_CONFIG = deep_merge(BASE_CONFIG, _RLOO_OVERRIDES)
